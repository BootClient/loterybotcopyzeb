class Giveaway {
    ...
    async init() {
        this.msg = await this.channel.buildEmbed()
            .setColor(0xcc8822)
            .setTitle(this.title)
            .setDescription(`Gagnants possible: ${this.winnerCount}\nRéagissez avec:tada: pour rentrer!`)
            .setFooter(`${this.suffix} | Fins dans`)
    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
            ...
            if (!list.length) {
                embed.description = `Gagnant: Aucun gagnant.`;
                embed.footer.text = `Giveaway Finis`;
            ...
            if (this.winners.length) this.msg.channel.send(`Bien joué, ${this.winners.map(u => u.toString()).join(", ")}! Tu viens de remporté le giveaway **${this.title}**!`);
        }
