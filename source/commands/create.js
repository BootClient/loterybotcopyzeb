...
module.exports = class extends Command {
    ...
    async execute(message) {
        if (!message.channel.permissionsFor(message.guild.me).has(['EMBED_LINKS', 'ADD_REACTIONS'])) return message.channel.send('Permissions invalide. Pour la simplicité, `EMBED_LINKS` et `ADD_REACTIONS` les autorisations sont nécessaires.');
        ...
        if (!match) return message.channel.send(`Utilisation non valide de la commande: \`${this.client.config.prefix}create [channel-mention|channel-id][\'-w <nombre-de-gagnants>\'] <#d#h#m#s|temps-en-secondes> <titre-du-giveaway>\``);
        ...
        if (filtered >= 5) return message.channel.send(`ERR: Vous ne pouvez avoir qu'un maximum de 5 cadeaux par canal.`);
