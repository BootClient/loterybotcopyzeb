...
function embed(input, output, error = false) {
    return new MessageEmbed().setColor(error ? 0xFF0000 : 0x00FF00).addField("Entrée", input).addField(error ? "Erreur" : "Sortie", `\`\`\`${error ? "" : "js"}\n${output}\n\`\`\``).setFooter("Vulnix. Giveaway");
}
module.exports = class extends Command {
    ...
    execute(message) {
        ...
        if (!code.length) return message.channel.send('Aucune entrée de code.');

        if (code.match(/token/gi)) return message.channel.send("L'entrée demande le token de l'utilisateur.");
